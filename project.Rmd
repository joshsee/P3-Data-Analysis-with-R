...
I find out that the first col X is a unique index variable for each individual observation. I think not very useful to the analysis so its best to remove it before procceding to the next step.
...
qplot(x=quality, data = wines, binwidth=1, 
      xlab='Quality', 
      ylab='Count',
...
I will start by doing a simple correlation test on quality versus other features and see whether we can get some insights about the dataset.

```{r echo=FALSE, message=FALSE, warning=FALSE, Bivariate_Plots_Section}
# fixed.acidity
with(subset(wines, fixed.acidity>=4 & fixed.acidity<=11), cor.test(fixed.acidity, quality, method = 'pearson'))
# -0.110
...
# citric.acid
with(subset(wines, citric.acid>=0 & citric.acid<=1), cor.test(citric.acid, quality_numeric, method='pearson'))
# -0.001
...
#Free.sulfur.dioxide
with(subset(wines, free.sulfur.dioxide>=0 & free.sulfur.dioxide<=90), cor.test(free.sulfur.dioxide, quality, method='pearson'))
# 0.031

# Total.sulfur.dioxide
with(subset(wines, total.sulfur.dioxide>=0 & total.sulfur.dioxide<=30), cor.test(total.sulfur.dioxide, quality, method='pearson'))
# 0.160

# Density
with(subset(wines, density>=0.98 & density<=1.01), cor.test(density, quality, method='pearson'))
# -0.317
...
# Alcohol
with(subset(wines, alcohol>=8 & alcohol<=14.20), cor.test(alcohol, quality, method='pearson')) 
# 0.436

```
It appears that alcohol, density, chlorides, total.sulfur.dioxide and volatile.acidity
...
p2 <- ggplot(aes(x = quality, y = density), data = wines) +
      geom_boxplot() +
      ggtitle('Quality vs Density')

p3 <- ggplot(aes(x = quality, y = chlorides), data = wines) +
      geom_boxplot() +
      ggtitle('Quality vs Chlorides')

p4 <- ggplot(aes(x = quality, y = volatile.acidity), data = wines) +
      geom_boxplot() +
      ggtitle('Quality vs Volatile.acidity')

p5 <- ggplot(aes(x = quality, y = total.sulfur.dioxide), data = wines) +
      geom_boxplot() +
      ggtitle('Quality vs total.sulfur.dioxide')

grid.arrange(p1,p2,p3,p4,p5, ncol = 2, main = "Relationship between Quality and 4 highest correlated variables \n")
```
...
ggplot(aes(x = alcohol, y = log10(residual.sugar)), data = wines) + 
       ...
       stat_smooth(method = 'lm') + 
       ggtitle('Log10 Density v. Residual Sugar')
