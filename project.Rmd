...
ggplot(aes(x = density, y = residual.sugar), 
       data = subset(wines, residual.sugar<quantile(wines$residual.sugar,.99) & 
        ...
        geom_vline(xintercept = mean(wines$density), linetype='longdash', color='blue', alpha=.5) +
        geom_hline(yintercept = mean(wines$residual.sugar), linetype='longdash', color='blue', alpha=.5) +
        guides(size=guide_legend(title="Quality Cut"), color=guide_legend(title="Quality Cut")) +
        ggtitle('Residual Sugar vs Density') +
        xlab("Density") +
        ylab("Residual Sugar")
...
When I swapped out alcohol with my defined new variable `quality` cut. The result is more obvious. We can see that better quality score wines are concentrated on the left hand side of the plot where as bad quality score wines are on the right.
...
ggplot(aes(x=alcohol), data = wines) +
  geom_bar(binwidth = 0.15. color = I('black'), fill = I('#099DD9')) +
  xlab("Alcohol") +
  ylab("Count") +
...
ggplot(aes(x = density, y = residual.sugar), 
       data = subset(wines, residual.sugar<quantile(wines$residual.sugar,.99) & 
                  density<quantile(density,.99))) +
        geom_jitter(alpha=.5, aes(size=quality_cut, color=quality_cut))
...
ggplot(data=wines, aes(x=alcohol, y=density, color=quality_as_factor)) +
    ...
    geom_point() +
    geom_vline(xintercept = mean(wines$alcohol), linetype='longdash', color='blue', alpha=.5) +
    geom_hline(yintercept = mean(wines$density), linetype='longdash', color='blue', alpha=.5)

```

### Description Three

In this graph, using the newly defined variable cut of quality score. We can see that good wines (8~10) concentrate in the lower right quadrant. That is when density is low and alcohol level is high. For lower quality wines, its concentrated in the upper left quadrant with high density and low alcohol level.   
...
fp2 <- ggplot(aes(x = quality, y = alcohol), data = wines) +
