...
ggplot(aes(x = alcohol, y = fixed.acidity, color=quality_as_factor), data = wines) + 
  geom_jitter(alpha=.3) +
  facet_wrap(~quality_cut) +
  guides(color=guide_legend(title="Quality"))
```
...
ggplot(aes(x = density, y = residual.sugar), 
  ...
  geom_vline(xintercept = mean(wines$density), linetype='longdash', color='blue', alpha=.5) +
  geom_hline(yintercept = mean(wines$residual.sugar), linetype='longdash', color='blue', alpha=.5) +
  ggtitle('Log 10 Residual Sugar vs Density vs Alcohol') +
  xlab("Density") +
  ylab("Log 10 Residual Sugar") +
  scale_y_log10()
