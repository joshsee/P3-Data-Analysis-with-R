...
# Since wine is just an integer, we can change quality scores as a factor, so that we can use it later
...
I decided to plot out ggpairs to have a look into the relationships. I found out that the top 4 most correclated variables with quality were alcohol, ph, chlorides, density.
...
From the boxplot above, we were able to understand more about the relationship between quality and other variables. From quality vs alcohol plot, we can clearly see that as the quality increases the alcohol level increases as well.
...
We already see a tendency in the boxplot, this can be better illustrate with a scatter plot and a linear regression line in it.
...
```{r echo=FALSE}
ggplot(aes(x = density, y = residual.sugar), data = subset(wines,
...
ggplot(data=wines, aes(x=alcohol, y=citric.acid)) +    
    ...
    coord_cartesian(
        ...
        ylim=c(quantile(wines$citric.acid,.01),quantile(wines$citric.acid,.99))
        )
```
