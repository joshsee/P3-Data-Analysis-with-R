...
The relationship between `density` and `residual.sugar` and `alcohol` seems quite interesting. Since all of them are highly correlated with one another, it is easy to spot the changes as any one of the variable varies. When we swapped out `alcohol` with `quality` score the same trend can be spotted as well.
