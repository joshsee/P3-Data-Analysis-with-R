...
wines$quality_cut <- cut(wines$quality, breaks=c(0,4,6,10))
```
...
I decided to plot out ggpairs to have a look into the relationships. I found out that the 4 most correclated variables with quality were alcohol, ph, chlorides, density.
...
From the boxplot above, we were able to understand more about the relationship between `quality` and other variables. In the alcohol vs quality plot, we can clearly see that as the `quality` increases the alcohol level increases as well. It has a mean of 10.514 and the mean increases as the quality score is above 5. Same goes with `pH`. It starts to increase when is above 5. `density` and `chlorides` shows a negative influence to `quality` score. Its means are 0.994 and 0.046 for `density` and `chlorides`, both average means decreases as the quality score is above 5.
...
We exploration further to find out the influence of main features with other secondary features.
...
I certainly would expect `fixed.acidity` to be correlated to `pH`. In chemistry when `acidity` goes down, the `pH` value should goes up as well.
...
Since our main goal was to find out what features affects `quality` score. The main relationship observed was `alcohol` and `quality`, `pH` and `quality`. Besides that `density` and `chlorides` also have negative influence on `qualty`. 
...
Since we found out that `density` and `residual.sugar` has the strongest relationship. I would like to explore this further with `alcohol`. 

```{r echo=FALSE}
ggplot(aes(x = pH, y = fixed.acidity), data = wines) + geom_jitter(alpha=.3, aes(color=quality_as_factor))
```
...
ggplot(data=wines, aes(x=alcohol, y=pH)) +
    ...
    geom_jitter(alpha=1/3, aes(size=quality.cut, color=quality.cut)) +
...
ggplot(data=wines, aes(x=alcohol, y=pH)) +    
    geom_jitter(alpha=1/2, aes(size=quality.cut, color=quality.cut)) +
    scale_x_continuous(limits = c(quantile(wines$alcohol, 0.01), 
                                quantile(wines$alcohol, 0.99))) +
    scale_y_continuous(limits = c(quantile(wines$pH, 0.01),
