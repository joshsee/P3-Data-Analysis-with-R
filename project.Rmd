...
I took a look at how correlated our different variables with quality:

```{r echo=FALSE}
cor(x=wines[,2:11],y=wines$quality)
```

I will concentrate in the first four variables that show strongest correlation with quality which are alcohol, ph, chlorides and density. 
...
p2 <- ggplot(aes(x = quality_as_factor, y = pH), data = wines) +
      geom_boxplot() +
      xlab("Quality") +
      ylab("pH") +
...
p4 <- ggplot(aes(x = quality_as_factor, y = density), data = wines) +
      geom_boxplot() +
      ylim(c(0.98, 1.01)) +
      xlab("Quality") +
      ylab("Density") +
...
grid.arrange(p1,p2,p3,p4, ncol = 2, main = "Relationship between Quality and 4 highest correlated variables \n")
```
...
```{r echo=FALSE}
ggplot(aes(x = as.numeric(quality), y = alcohol), data = wines) + 
