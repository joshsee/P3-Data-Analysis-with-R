...
```{r echo=FALSE, message=FALSE, warning=FALSE, Boxplot_of_Quality_vs_all_Other_variable}
...
ggplot(aes(x = pH, y = residual.sugar), data = wines) + 
  ...
  ylim(c(0, 20)) + 
  scale_y_log10() +
  ggtitle('log 10 Residual Sugar vs pH') +
  xlab("pH") +
...
ggplot(aes(x = density, y = residual.sugar), data = 
        ...
        geom_point(alpha = 1/5) +       
        geom_smooth(method = 'lm') + 
        ggtitle('log 10 Residual Sugar vs Density') +
        xlab("Density") +
        ylab("log 10 Residual Sugar") +
        scale_y_log10()
...
ggplot(aes(x = pH, y = fixed.acidity), data = wines) + 
  ...
  xlab("pH") +
  ylab("fixed.acidity") +
  ggtitle('pH vs Fixed.acidity vs Quality Cut')
